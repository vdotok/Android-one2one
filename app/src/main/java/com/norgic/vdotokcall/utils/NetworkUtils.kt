...
import com.norgic.vdotokcall.network.ParsedError
...
suspend fun <T : Any> request(call: suspend () -> Result<T>): Result<T> {
    return try {
        ...
        Result.Error(ParsedError(e.message.orEmpty(), ApplicationConstants.HTTP_CODE_NO_NETWROK))
    }
}
    ...
    return request {
        with(onRequest()) {
            if (isSuccessful) {
                ...
                Result.Error(ParsedError(getLogMessage(this), ApplicationConstants.HTTP_CODE_NO_NETWROK))
            }
        }
    }
}
